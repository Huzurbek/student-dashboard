<template>
  ...
  <div v-if="done.length < tests.length">
    ...
    <div class="answer-box" v-for="(option, index) in tests[currentQuestion].variants" :key="index">
      <input type="checkbox" class="check-box" :checked="option==pick" @change="(pick=pick==option?null:option)">
  ...
  <div v-if="tests.length && done.length >= tests.length">
...
export default {
  ...
  computed: {
    ...mapState({
      testResult: (state) => state.courseModule.activeLesson.tests,
      done: (state) => state.courseModule.done,
    }),
    tests() {
      return this.testResult ? this.testResult : {}
    }
    // ...mapGetters({
    //   tests: 'courseModule/getTests'
    // })
  },
  methods: {
    submit() {
      if (this.pick[0] === '@') {
        this.$store.commit("courseModule/pushDone", 'correct');
      } else {
        this.$store.commit("courseModule/pushDone", 'incorrect');
      }
      this.pick = ''
